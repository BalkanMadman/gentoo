...
# Zurab Kvachadze <zurabid2016@gmail.com>
# @SUPPORTED_EAPIS: 8
# @PROVIDES: edo multiprocessing perl-functions systemd toolchain-funcs tmpfiles
...
# version of the nginx-tests[1] repository.
...
# not be set for the NGINX versions that do not have the modules in question.
...
# enables NGINX modules and subsystems listed in NGINX_MODULES and
...
nginx_src_configure() {
	...
	# not recognise arguments like '--with-http_rewrite_module', it understands
	# only '--without-http_rewrite_module', as http_rewrite module is enabled by
	# default.
	local subsys mod def_state use_state
	for subsys in "${_NGX_SUBSYSTEMS[@]}"; do
		use "${subsys#+}"; use_state=$?
		# If the first character is '+' which means the subsytem is
		# enabled-by-default, the default state is 0. If the first character is
		# not '+', the default state is 1 - the subsytem is disabled-by-default.
		[[ ${subsys:0:1} == + ]]; def_state=$?
		# If the USE flag does not correspond to the default value of the
		# subsytem (e.g. a subsytem is default-enabled, but the corresponding
		# USE flag is disabled), we add the respective flag to the list of
		# flags. An example:
		#
		#	- subsytem 'http' is enabled by default. The user does not want the
	...
	for mod in "${_NGX_MODULES[@]}"; do
		...
		#
		#   - Sadly, module 'http_rewrite' is not enabled by default. The user
		#   has absolutely no need for regular expressions, and, logically, does
		#   not enable the corresponding 'nginx_modules_http_rewrite' USE flag.
		#   As such, use_state will be '1' and so will be def_state. The former
		#   yields '1', as the USE flag is off and the latter equals to '1' as
		#   the 'http_rewrite' module does not have a '+' in front of it in
		#   _NGX_MODULES array, signifying its default-disabled state. Due to
		#   default state being equal to the USE state, no flag will be passed
		#   to ./configure.
		#
		...
		#   '1', as no '+' is present in front of 'http_ssl' in _NGX_MODULES
		#   array. This denotes the fact that 'http_ssl' is disabled by default.
		#   Since use_state and def_state are not equal, which means that the
		#   default and the actual state are different, '--with-http_ssl_module'
...
nginx_src_install() {
	...
	if use modules; then
		...
		# status, irregardless of what the last statement evaluates to.
